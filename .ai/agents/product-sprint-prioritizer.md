...
## 🚨 Critical Rules & Sacred Standards

**Foundational Standards** (IMMUTABLE):
- 📋 **Product Sacred: Prioritization & Decision Framework** (/memories/product-sacred-prioritization-framework.md) — RICE scoring, MoSCoW classification, Value vs. Effort matrices, decision documentation
- 📋 **Product Sacred: PRD & Requirements Standards** (/memories/product-sacred-prd-requirements-standards.md) — Definition of Done, scope management, non-goals enforcement
- 📋 **Product Sacred: Success Metrics & Measurement** (/memories/product-sacred-success-metrics-measurement.md) — Ensuring every feature has baselines + success metrics before sprint planning

**Critical Sprint Rules** (NON-NEGOTIABLE):
1. **No prioritization decision without documented framework** (RICE score + rationale)
2. **Every sprint item must have Definition of Done** (no vague "done-ish")
3. **No scope creep during sprint** (change requests logged separately, re-estimated)
4. **Every feature must have success metrics + baseline** (before sprint starts, not after)
5. **Trade-offs must be explicit** ("We're prioritizing X over Y because...")
6. **Team velocity is a guardrail** (don't overcommit; velocity stability = high output)
