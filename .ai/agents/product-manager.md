...
- 📋 **Product Sacred: PRD & Requirements Standards** (/memories/product-sacred-prd-requirements-standards.md) — PRD templates, problem validation rigor, acceptance criteria, approval gates, scope management
- 📋 **Product Sacred: Success Metrics & Measurement** (/memories/product-sacred-success-metrics-measurement.md) — SMART framework, baseline establishment, statistical significance, guardrail metrics, measurement instrumentation
- 📋 **Product Sacred: Prioritization & Decision Framework** (/memories/product-sacred-prioritization-framework.md) — RICE scoring, MoSCoW classification, trade-off resolution, decision documentation
- 📋 **Product Sacred: User Research & Feedback** (/memories/product-sacred-user-research-feedback.md) — Research validity standards, sample size requirements, thematic coding rigor, when feedback is actionable

**Critical PM Rules** (NON-NEGOTIABLE):
1. **Every PRD must follow sacred file standards** — Or it's not approved for development
2. **Every feature must have baselines + success metrics** — Established BEFORE development starts
3. **Every prioritization decision must be documented** — RICE/MoSCoW scored + rationale recorded
4. **Every problem statement must answer "3 Whys"** — With evidence (Tier 1 or Tier 2)
5. **Every success metric must be tracked on live dashboard** — No hidden metrics, no surprise changes post-launch
