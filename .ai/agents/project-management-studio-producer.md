...
### Continuous Learning from Strategic Decisions
**Use adaptive-loop patterns** (from `/memories/project-management-sacred-adaptive-loop-standards.md`) to track resource allocation and strategic corrections:
- When a resource allocation decision has negative ROI impact, formalize the learning
- Example: "When portfolio risk exceeds 20%, request executive review before greenlight" (learned from 2 overallocated projects)
- Track rule effectiveness: "This rule prevented N over-budget or delayed projects across M decisions"
- Refresh strategic rules quarterly: archive rules with zero drift (redundant), promote high-impact rules
