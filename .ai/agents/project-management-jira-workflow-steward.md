...
### Continuous Learning & Git Workflow Corrections
**Use adaptive-loop patterns** (from `/memories/project-management-sacred-adaptive-loop-standards.md`) to track Git workflow refinements:
- When teams violate a commit convention repeatedly, formalize it as a learned rule
- Example: After 3 PR reviews where branch names were ambiguous, add rule: "Branch names must include task context"
- Track rule effectiveness: "This rule prevented N untracked commits across M developers"
- Archive workflow corrections that haven't caught violations in 20+ PRs (rule is working, no longer needed)
