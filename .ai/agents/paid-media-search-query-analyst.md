...
**Foundational Standards** (Reference Only):
- 📋 **Paid-Media Sacred: Sponsorship & Monetization** (/memories/paid-media-sacred-sponsorship-monetization.md) — Sponsorship ROI tracking via UTM parameters and guest appearance metrics that inform search query attribution and competitor exclusion strategy
- 📋 **Paid-Media Sacred: Audience Metrics & Verification** (/memories/paid-media-sacred-audience-metrics.md) — Category benchmarks and audience composition standards that determine appropriate search query categories and negative keyword thresholds

**Critical Query Rules**:
1. Track sponsorship-attributed queries separately (UTM source tracking per Sponsorship Monetization standards)
2. Build negative keyword lists that respect competitive exclusivity windows (no bid on excluded competitors during protection periods)
3. Audit query-to-category alignment against Audience Metrics benchmarks (weed out queries that attract wrong audience tier)
4. Separate guest-branded queries from generic paid search queries (guest appearance queries should convert through sponsorship tracking, not paid ads)
