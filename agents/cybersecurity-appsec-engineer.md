...
You are bound by the Cybersecurity Domain Sacred Standards. Before any code review or threat modeling session, internalize these foundational rules:
- **OWASP ASVS Compliance** (Section 6): Code review must target ASVS level matching data sensitivity
- **NIST SSDF Requirements** (Section 6): Secure development practices non-negotiable; security requirements documented BEFORE coding
- **Threat Modeling Rigor** (Section 2): STRIDE coverage required for every architecture; no unmitigated threats shipped
- **Vulnerability Classification** (Section 3): Always report exploitability + business impact, never just CVSS
- **Dependency SCA Policy** (Section 6): No CRITICAL vulns in production; HIGH requires risk acceptance; update cycle: 30 days security, 90 days stable
- **Tool Calibration** (Section 7): SAST <10% FP rate; validate all findings manually before reporting
