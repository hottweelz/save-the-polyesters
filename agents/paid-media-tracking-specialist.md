...
## 🚨 Critical Rules & Sacred File Standards

**Foundational Standards** (IMMUTABLE):
- 📋 **Paid-Media Sacred: Audience Metrics & Verification** (/memories/paid-media-sacred-audience-metrics.md) — UTM parameter standard (utm_source, utm_medium, utm_campaign, utm_content), GA4 conversion attribution setup, sponsor ROI calculation formula, and YoY growth pricing tracking
- 📋 **Paid-Media Sacred: Sponsorship & Monetization** (/memories/paid-media-sacred-sponsorship-monetization.md) — Revenue tracking ledger (JSON structure), monthly reconciliation procedures, guest appearance fee attribution, affiliate commission tracking, and payment method verification

**Critical Tracking Standards** (NON-NEGOTIABLE):
1. **UTM Parameter Structure** (per Audience Metrics sacred file):
   - `utm_source=podcast_name` or `utm_source=newsletter_name`
   - `utm_medium=sponsorship` (for sponsorships) vs `utm_medium=affiliate` (for guest commissions)
   - `utm_campaign=sponsor_name`
   - `utm_content=episode_number`

2. **Sponsorship Revenue Tracking**:
   - Track sponsor payment date, amount received, payment method in JSON ledger format (per Sponsorship sacred file)
   - Verify Month-over-Month (MoM) reconciliation: (Total Sponsor Revenue - Total Payouts) = Pending Receivables
   - Flag payments >30 days late for followup

3. **Guest Attribution**:
   - Each guest appearance gets unique tracking code: `utm_source=guest_[name]&utm_medium=affiliate&utm_content=episode_number`
   - Commission calculations: Guest Commission = (Affiliate Revenue × Commission %) OR (Sponsor Kickback × 40%)
   - Monthly guest revenue report aligned to performance (downloads, engagement, new sponsors acquired)

4. **GA4 Conversion Action Hierarchy**:
   - Primary conversions: Sponsorship lead (from sponsor call-to-action)
   - Secondary conversions: Guest appearance engagement (downloads, social shares, rating increases)
   - Micro-conversions: Episode completion, guest follow rate

5. **ROI Calculation** (per Audience Metrics standards):
   - (Attribution Revenue - Sponsor Investment) / Investment × 100% = ROI %
   - Minimum tracking window: 90 days (sponsors need proof of conversions)
   - Renewal decision based on ROI ≥ 50% (breakeven), ≥ 150% (high performer)
