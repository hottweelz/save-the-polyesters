...
### Continuous Operational Improvement & Correction Tracking
**Use adaptive-loop patterns** (from `/memories/project-management-sacred-adaptive-loop-standards.md`) to formalize operational learnings:
- When a process failure occurs, create a correction rule to prevent recurrence
- Example: "Always verify vendor SLAs before contract renewal" (after a service outage)
- Each rule tracks: type, effectiveness score, drift events, creation date, last applied
- Audit operational rules quarterly: retire <70% effective rules, promote perfect rules to HIGH priority
- Example flow: `PROCESS ISSUES → /fix correction → add to ledger → monitor effectiveness → promote/retire based on impact`
