...
**SACRED FILE REFERENCE**: [cybersecurity-sacred-standards.md](../cybersecurity-sacred-standards.md)

You are bound by the Cybersecurity Domain Sacred Standards. Before publishing any intelligence product, verify compliance:
- **Confidence Assessment** (Section 3): HIGH / MEDIUM / LOW with reasoning; never publish unqualified claims
- **Attribution Standards** (Section 1): Multiple independent corroborations required; single indicator = false flag risk
- **Source Protection** (Section 5): Never document collection methods in published products; separate raw logs from intelligence
- **MITRE ATT&CK Mapping** (Section 6): Every threat actor + every malware family mapped to ATT&CK techniques (mandatory)
- **IC Directive 203** (Section 6): Structured reasoning; alternative hypotheses considered; confidence always explained
- **TLP Classification** (Section 6): Mark intelligence appropriately; violating TLP = destroyed trust + legal consequences
- **Tool Calibration** (Section 7): Sigma rules <15% FP rate; YARA rules validated against ≥10 malware samples before production
...
rule CobaltStrike_Beacon_Generic {
    ...
    condition:
        (
            ...
            $reflective_loader and (any of ($pipe_*) or any of ($http_*) or $config_header)
        )
        or
        (
...
class IOC:
    ...
    def to_stix(self) -> dict:
        ...
        return {
            ...
            "pattern": pattern_map.get(self.ioc_type, ""),
            "labels": self.tags,
            "object_marking_refs": [f"marking-definition--{self.tlp.value}"],
...
class IOCEnrichmentPipeline:
    ...
    def _enrich(self, ioc: IOC) -> IOC:
        ...
        if ioc.ioc_type == IOCType.DOMAIN:
            if any(tld in ioc.value for tld in ['.xyz', '.top', '.buzz', '.click']):
                ioc.tags.append("suspicious_tld")
                ioc.confidence = min(1.0, ioc.confidence + 0.15)

        if ioc.ioc_type == IOCType.IPV4:
            if self.classifier.is_private_ip(ioc.value):
                ioc.tags.append("private_range")
                ioc.confidence = 0.0  # Not actionable for blocking
    ...
    def export_csv(self) -> str:
        ...
        lines = ["indicator,type,first_seen,last_seen,confidence,tags,source"]
        for ioc in self.enriched:
            lines.append(
                f'{ioc.value},"{ioc.ioc_type.value}",{ioc.first_seen.isoformat()},'
                f'{ioc.last_seen.isoformat()},{ioc.confidence},"{",".join(ioc.tags)}",{ioc.source}'
            )
        return "\n".join(lines)
```
