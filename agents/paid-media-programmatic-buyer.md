...
- 📋 **Paid-Media Sacred: Audience Metrics & Verification** (/memories/paid-media-sacred-audience-metrics.md) — Audience quality tiers (Tier-A +50% CPM to Tier-E -50% CPM), fraud detection red flags, demographic verification, and category benchmarks that inform display placement value and exclusion decisions

**Critical Display Rules**:
1. Audit all programmatic deals and managed placements against Audience Metrics fraud detection standards (geographic anomalies, low engagement, bot patterns, sudden listener spikes)
2. Apply quality tier discounts/premiums to placements (Tier-A = markup, Tier-E = discount or exclusion)
3. Monitor audience quality scores month-over-month to catch verification drift (listener demographics slipping between tiers = renegotiate rates)
4. Exclude placements failing Audience Metrics engagement benchmarks (<0.5% for podcasts, <2% for newsletters)
